...
//                }
                    ...
                    InstructionDAO info = InstructionDBBuilder.getInstruction(instruction.getOpcode());
                    if (info == null)
    ...
    private InstructionDAO analyzeDelegate(InstructionDAO info, CS2FlowBlock block, CS2Stack stack) {
        ...
        return new InstructionDAO(info.getOpcode(), name, InstructionDAO.getPopOrder(args), names, CS2Type.VOID);
    ...
    private InstructionDAO analyzeSpecialCall(Instruction instruction, 
            InstructionDAO info, CS2FlowBlock block, CS2Stack stack) {
        if (instruction.getName().equals("_db_getfield")) {
            ...
            if (rtypes.length == 0)
                return new InstructionDAO(info.getOpcode(), info.getName(), info.getPopOrder(), info.getArgumentNames(), CS2Type.VOID);
            else if (rtypes.length == 1)
                return new InstructionDAO(info.getOpcode(), info.getName(), info.getPopOrder(),
                        info.getArgumentNames(), rtypes[0]);
            else {
                String s = "(";
                for (int i = 0; i < rtypes.length; i++)
                ...
                s += ")";
                return new InstructionDAO(info.getOpcode(), info.getName(), info.getPopOrder(),
                        info.getArgumentNames(), CS2Type.forDesc("dbfield" + s));
            }
        }
            ...
            return new InstructionDAO(info.getOpcode(), info.getName(), info.getPopOrder(),
                    info.getArgumentNames(), CS2Type.forJagexId(((CS2PrimitiveExpression)args[1]).asInt()));
        }
            ...
            if (((CS2PrimitiveExpression)args[0]).asInt() > 700 || ((CS2PrimitiveExpression)args[1]).asInt() > 700)
                return new InstructionDAO(info.getOpcode(), info.getName(), info.getPopOrder(),
                        info.getArgumentNames(), CS2Type.forDesc("random_pitch_sound(2,0,0)"));
            else
                return new InstructionDAO(info.getOpcode(), info.getName(), info.getPopOrder(),
                        info.getArgumentNames(), CS2Type.INT);
        }
            ...
            return new InstructionDAO(info.getOpcode(), info.getName(), info.getPopOrder(),
                    info.getArgumentNames(), CS2Type.forJagexChar((char)paramtypes[paramId]));
        }
            ...
            return new InstructionDAO(info.getOpcode(), info.getName(), InstructionDAO.getPopOrder(argtypes), argnames, rtype);
        }
        else
