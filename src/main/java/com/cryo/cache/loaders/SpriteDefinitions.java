...
public final class SpriteDefinitions {
	...
	public void decodeArchive(Store cache, int archiveId, int fileId) {
		...
		stream.setOffset(0);
		for (int spriteIndex = 0; spriteIndex < images.length; spriteIndex++) {
			int pixelsIndexesLength = imagesWidth[spriteIndex] * imagesHeight[spriteIndex];
			pixels[spriteIndex] = new int[pixelsIndexesLength];
			alpha[spriteIndex] = new byte[pixelsIndexesLength];
			int maskData = stream.readUnsignedByte();
			if ((maskData & 0x2) == 0) {
				if ((maskData & 0x1) == 0) {
					for (int index = 0; index < pixelsIndexesLength; index++) {
						pixels[spriteIndex][index] = (byte) stream.readByte();
					}
				} else {
					for (int i_24_ = 0; i_24_ < imagesWidth[spriteIndex]; i_24_++) {
						for (int i_25_ = 0; i_25_ < imagesHeight[spriteIndex]; i_25_++) {
							pixels[spriteIndex][i_24_ + i_25_ * imagesWidth[spriteIndex]] = (byte) stream.readByte();
						}
					}
				}
			} else {
				usesAlpha[spriteIndex] = true;
				boolean bool = false;
				if ((maskData & 0x1) == 0) {
					for (int index = 0; index < pixelsIndexesLength; index++) {
						pixels[spriteIndex][index] = (byte) stream.readByte();
					...
					for (int i_27_ = 0; i_27_ < pixelsIndexesLength; i_27_++) {
						byte i_28_ = (alpha[spriteIndex][i_27_] = (byte) stream.readByte());
						bool = bool | i_28_ != -1;
					}
				} else {
					for (int i_29_ = 0; i_29_ < imagesWidth[spriteIndex]; i_29_++) {
						for (int i_30_ = 0; i_30_ < imagesHeight[spriteIndex]; i_30_++) {
							pixels[spriteIndex][i_29_ + i_30_ * imagesWidth[spriteIndex]] = stream.readByte();
						}
					}
					for (int i_31_ = 0; i_31_ < imagesWidth[spriteIndex]; i_31_++) {
						for (int i_32_ = 0; i_32_ < imagesHeight[spriteIndex]; i_32_++) {
							byte i_33_ = (alpha[spriteIndex][i_31_ + i_32_ * imagesWidth[spriteIndex]] = (byte) stream.readByte());
							bool = bool | i_33_ != -1;
						}
					}
				}
				if (!bool)
					alpha[spriteIndex] = null;
			}
			images[spriteIndex] = getBufferedImage(imagesWidth[spriteIndex], imagesHeight[spriteIndex], pixels[spriteIndex], alpha[spriteIndex], usesAlpha[spriteIndex]);
		}
	}
	...
	public byte[] encodeFile() {
		...
		for (int imageId = 0; imageId < images.length; imageId++) {
			...
			for (int index = 0; index < pixels[imageId].length; index++)
				stream.writeByte(pixels[imageId][index]);
			if (usesAlpha[imageId])
	...
	public void generatePallete() {
		...
		for (int index = 0; index < images.length; index++) {
			...
			pixels[index] = new int[image.getWidth() * image.getHeight()];
			alpha[index] = new byte[image.getWidth() * image.getHeight()];
			for (int pixel = 0; pixel < pixels[index].length; pixel++) {
